...
# -*- coding: utf-8 -*-
"""
Context-Engineering：用于多步骤 LLM 交互的控制循环
=================================================

本模块演示了如何实现控制流机制，以编排复杂的多步骤 LLM 交互。
在前面几个 notebook 所介绍的上下文扩展技术基础上，我们现在进一步
探索以下模式：

1. 顺序链式处理（上一步的输出 → 下一步的输入）
2. 迭代优化（通过多轮循环持续改进回答）
3. 条件分支（根据 LLM 输出选择不同路径）
4. 自我批评与纠正（对输出进行元评估）
5. 外部验证循环（使用工具/知识进行校验）

这些模式的实现重点关注 token 效率，以及在多步骤之间维持上下文一致性。
...
def setup_client(api_key=None, model=DEFAULT_MODEL):
    ...
    if api_key is None:
        ...
        if api_key is None and not ENV_LOADED:
            logger.warning(
                "No API key found. Set OPENAI_API_KEY env var or pass api_key param."
            )
        ...
        metadata.update(
            {
                ...
                "token_efficiency": (
                    response_tokens / (prompt_tokens + system_tokens)
...
def display_response(
    prompt: str, response: str, metrics: Dict[str, Any], show_prompt: bool = True
) -> None:
    """
...
class ControlLoop:
    ...
    def __init__(
        ...
        self.client, self.model = (
            setup_client(model=model) if client is None else (client, model)
        )
    ...
    def _call_llm(
        self, prompt: str, custom_system_message: Optional[str] = None
    ) -> Tuple[str, Dict[str, Any]]:
        """
        ...
        """
        system_msg = (
            custom_system_message if custom_system_message else self.system_message
        )
    ...
    def get_summary_metrics(self) -> Dict[str, Any]:
        ...
        # Add derived metrics
        if summary["steps"] > 0:
            summary["avg_latency_per_step"] = (
                summary["total_latency"] / summary["steps"]
            )
    ...
    def visualize_metrics(self) -> None:
        ...
        # Plot 1: Token usage
        axes[0, 0].bar(
            steps, prompt_tokens, label="Prompt Tokens", color="blue", alpha=0.7
        )
        axes[0, 0].bar(
            steps,
            response_tokens,
        ...
        cumulative_tokens = np.cumsum(
            [h["metrics"].get("total_tokens", 0) for h in self.history]
        )
...
class IterativeRefiner(ControlLoop):
    ...
    def refine_response(
        self, previous_response: str, feedback: str
    ) -> Tuple[str, Dict[str, Any]]:
        """
        ...
        prompt = self.refinement_template.format(
            previous_response=previous_response, feedback=feedback
        )
        return self._call_llm(prompt)

    def run(
        self, initial_prompt: str, use_auto_feedback: bool = True
        ...
        while should_continue and iteration < self.max_iterations:
            ...
            # Refine response
            refined_response, refine_metadata = self.refine_response(
            ...
            if self.stopping_condition:
                should_continue = not self.stopping_condition(
                    current_response, refine_metadata
                )
...
class ConditionalBrancher(ControlLoop):
    ...
    def classify_input(self, input_text: str) -> Tuple[str, Dict[str, Any]]:
        ...
        prompt = self.classifier_template.format(
            categories=categories_str, input=input_text
        )

        # Use a specific system message for classification
        system_message = (
            "You are a classifier that categorizes inputs precisely and accurately."
        )
        response, metadata = self._call_llm(prompt, system_message)
        ...
        self._log(
            f"Warning: Could not classify input. Using first branch: {categories[0]}"
        )
        return categories[0], metadata

    def execute_branch(
        self, branch_name: str, input_text: str
    ) -> Tuple[str, Dict[str, Any]]:
        """
    ...
    def run(
        self, input_text: str, branch_name: Optional[str] = None
    ) -> Tuple[str, Dict[str, Any]]:
        """
    ...
    def run(self, input_text: str) -> Tuple[str, Dict[str, Any]]:
        ...
        for attempt in range(1, self.max_attempts + 1):
            ...
            if attempt >= self.max_attempts:
                self._log(
                    f"Max attempts ({self.max_attempts}) reached without valid response"
                )
                break
    ...
    def display_results(self, run_details: Dict[str, Any]) -> None:
        ...
        for attempt_data in run_details["attempts"]:
            ...
            if not attempt_data["validation"]["pending"]:
                ...
                if is_valid:
                    display(
                        HTML("<p style='color: green; font-weight: bold;'>✓ Valid</p>")
                    )
                else:
                    display(
                        HTML("<p style='color: red; font-weight: bold;'>✗ Invalid</p>")
                    )
                    display(HTML("<h4>Feedback:</h4>"))
...
def example_iterative_refiner():
    ...
    refiner = IterativeRefiner(
        max_iterations=3, stopping_condition=quality_threshold, verbose=True
    )
...
def example_external_validation():
    ...
    validator = ExternalValidation(
        validator_fn=python_validator, max_attempts=3, verbose=True
    )
