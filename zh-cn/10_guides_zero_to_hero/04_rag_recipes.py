...
# -*- coding: utf-8 -*-
"""
Context-Engineering：用于检索增强生成（RAG）的实践方案
================================================================

本模块演示了检索增强生成（RAG）模式的实用实现，用于通过外部知识
增强 LLM 的上下文。我们重点关注简洁、高效的实现方式，在不依赖复杂
基础设施的前提下突出关键概念。
    ...
    def _retrieve(
        ...
        for doc, score in keyword_results:
            if doc.id not in combined_scores:
                ...
                combined_scores[doc.id]["keyword_score"] = score

        hybrid_results = []
        for entry in combined_scores.values():
            hybrid_score = (
                self.embedding_weight * entry["semantic_score"]
                + self.keyword_weight * entry["keyword_score"]
            )
            hybrid_results.append((entry["doc"], hybrid_score))

        hybrid_results.sort(key=lambda item: item[1], reverse=True)
